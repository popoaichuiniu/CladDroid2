...
public abstract class BaseEntryPointCreator implements IEntryPointCreator {
	...
	protected SootMethod createEmptyMainMethod(Body body){
		...
		if (Scene.v().containsClass(dummyClassName)) {//protected String dummyClassName = "dummyMainClass";
			int methodIndex = 0;
			...
			mainClass = new SootClass(dummyClassName);//新建一个dummyMainClass加入到scene
		...
		SootMethod mainMethod = new SootMethod(methodName,
				Collections.singletonList(stringArrayType), VoidType.v());//新建一个返回值为空 ,参数为长度为1的String数组的参数的"dummyMainMethod"方法
		...
		mainClass.setApplicationClass();//这个将会被分析application和argument ，library class区别
		mainMethod.setModifiers(Modifier.PUBLIC | Modifier.STATIC);
		return mainMethod;//这里就是生成了一个包含public static void dummyMainMethod (String [] args)的方法。细节指出可能不对
	}
