...
public abstract class AbstractInfoflow implements IInfoflow {
	...
	protected void constructCallgraph() {
		...
		ipcManager.updateJimpleForICC();//默认的IPCManager啥都没干

		// Run the preprocessors
        for (PreAnalysisHandler tr : preProcessors)//Handler that allows clients to execute certain tasks before the actual data flow analysis is executed.
            tr.onBeforeCallgraphConstruction();
        
        // Patch the system libraries we need for callgraph construction
        LibraryClassPatcher patcher = new LibraryClassPatcher();//Class for patching OS libraries such as java.lang.Thread so that we get complete callgraphs
        //哪里有介绍详细的类，怎么知道缺少啥
        patcher.patchLibraries();
		
        // To cope with broken APK files, we convert all classes that are still
        // dangling after resolution into phantoms
        for (SootClass sc : Scene.v().getClasses())//Returns an backed chain of the classes in this manager.
        	if (sc.resolvingLevel() == SootClass.DANGLING) {//Soot representation of a Java class. They are usually created by a Scene, but can also be constructed manually through the given constructors.
        		sc.setResolvingLevel(SootClass.BODIES);
        		sc.setPhantomClass();//phantom class???
        	}
