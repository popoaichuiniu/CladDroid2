...
//    }
    ...
    public static void main(String[] args) {
        ...
        if (appDirFile.isDirectory()) {
            ...
            File hasSatisticIfReducedAndPreviousIFFile = new File(Config.intentConditionSymbolicExcutationResults + "/" + appDirFile.getName() + "_hasSatisticIfReducedAndPreviousIF.txt");
            if (!hasSatisticIfReducedAndPreviousIFFile.exists()) {
                try {
                    hasSatisticIfReducedAndPreviousIFFile.createNewFile();
                } catch (IOException e) {
                    exceptionLogger.error("无法创建hasSatisticIfReducedAndPreviousIF文件!");
                }

            }
            if (hasSatisticIfReducedAndPreviousIFFile.exists()) {
                Set<String> hasAnalysisAPP = new ReadFileOrInputStream(hasSatisticIfReducedAndPreviousIFFile.getAbsolutePath(), exceptionLogger).getAllContentLinSet();
                    ...
                    if ((!file.getName().contains("_signed_zipalign")) && file.getName().endsWith(".apk")) {
                        ...
                        Thread childThread = new Thread(new Runnable() {
                            ...
                            public void run() {
                                ...
                                WriteFile writeFileHasBeenProcessedApp = new WriteFile(hasSatisticIfReducedAndPreviousIFFile.getAbsolutePath(), true, exceptionLogger);
